...
// ***********************************************************************
// <copyright file="ExportUtil1.razor.cs" company="Notes2022RCL">
...
#pragma warning restore CA1816 // Dispose methods should call SuppressFinalize
        ...
        //            List<Tags> tags = new(); //await Http.GetFromJsonAsync<List<Tags>>("api/Tags/" + nfid);
        //            if (isHtml)
        //            {
        //                // Start the document
        ...
        //            foreach (NoteHeader bnh in bnhl.List)
        //            {
        ...
        //                    NoteHeaderList zz1 = await Client.GetExportAsync(new ExportRequest()
        ...
        private async Task<MemoryStream> DoExportFast()
        {
            ...
            //bnhl = await Client.GetExportAsync(new ExportRequest() { FileId = nfid, ArcId = model.ArchiveNumber, NoteOrdinal = NoteOrd, ResponseOrdinal = 0 }, myState.AuthHeader);
            // loop over each base note in order
            foreach (NoteHeader bnh in baseNotes)
            {
                if (bnh.IsDeleted || bnh.Version > 0)
                    continue;
                if (marked && !model.Marks.Where(p => p.NoteOrdinal == bnh.NoteOrdinal).Any())
                ...
                if (isCollapsible && isHtml && bnh.ResponseCount > 0)
                {
                    await sw.WriteLineAsync("<div class=\"container\"><div class=\"panel-group\">" + "<div class=\"panel panel-default\"><div class=\"panel-heading\"><div class=\"panel-title\"><a data-toggle=\"collapse\" href=\"#collapse" + bnh.NoteOrdinal + "\"><h5>Toggle " + bnh.ResponseCount + " Response" + (bnh.ResponseCount > 1 ? "s" : "") + "</h5></a></div></div><div id = \"collapse" + bnh.NoteOrdinal + "\" class=\"panel-collapse collapse\"><div class=\"panel-body\">");
                }
...
#pragma warning restore CS8600 // Converting null literal or possible null value to non-nullable type.
                    ...
                    //NoteHeaderList zz1 = await Client.GetExportAsync(new ExportRequest()
                    //{ FileId = nfid, ArcId = model.ArchiveNumber, NoteOrdinal = bnh.NoteOrdinal, ResponseOrdinal = rnum }, myState.AuthHeader);
                    List<NoteHeader> zz1 = allHeaders.Where(p => p.NoteOrdinal == bnh.NoteOrdinal && p.ResponseOrdinal != 0).ToList();
                    nh = zz1[rnum - 1];
                    if (!nh.IsDeleted && nh.Version == 0)
                    {
                        //req = nh.Id.ToString();
        ...
        private static async Task<bool> WriteNote(StreamWriter sw, NoteHeader bnh, NoteHeader nh, NoteContent nc, bool isHtml, bool isResponse, List<Tags> tags)
        {
            //StringBuilder sb;
            if (isHtml)
            {
            ...
            if (!isResponse)
            {
                ...
                await sw.WriteLineAsync("Note: " + nh.NoteOrdinal + " - Subject: " + nh.NoteSubject + (isHtml ? "<br />" : " - ") + "Author: " + nh.AuthorName + " - " + nh.LastEdited.ToDateTime().ToShortDateString() + " " + nh.LastEdited.ToDateTime().ToShortTimeString() + " - " + bnh.ResponseCount + " Response" + (bnh.ResponseCount > 1 || bnh.ResponseCount == 0 ? "s" : ""));
            }
            else
            {
                // write response note header
                await sw.WriteLineAsync("Note: " + nh.NoteOrdinal + " - Subject: " + nh.NoteSubject + (isHtml ? "<br />" : " - ") + "Author: " + nh.AuthorName + " - " + nh.LastEdited.ToDateTime().ToShortDateString() + " " + nh.LastEdited.ToDateTime().ToShortTimeString() + " - " + "Response " + nh.ResponseOrdinal + " of " + bnh.ResponseCount);
