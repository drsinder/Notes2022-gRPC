            ...
            string userId, string name, string title)
        {
            ...
            if (!query.Any())
            {
                NoteFile noteFile = new()
                {
                    ...
                    Id = 0,
                    OwnerId = userId,
                    LastEdited = Google.Protobuf.WellKnownTypes.Timestamp.FromDateTime(DateTime.UtcNow)
                };
...
//                dbTran.Commit();
        ...
        public static async Task<NoteHeader> CreateResponse(NotesDbContext db, NoteHeader nh, string body, string tags, string dMessage, bool send, bool linked, bool editing = false)
        {
            ...
            mine.ThreadLastEdited = Google.Protobuf.WellKnownTypes.Timestamp.FromDateTime(DateTime.UtcNow);
            mine.ResponseCount++;
